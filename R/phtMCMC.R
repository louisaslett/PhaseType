phtMCMC = function(x, states, beta, nu, zeta, n, mhit=1, resume=NULL, silent=FALSE) {
	...
	# Form TT
	TT <- matrix(paste("S", 1:states, rep(1:states, each=states), sep = ""), nrow=states, ncol=states)
	TT <- cbind(TT, paste("s", 1:states, sep = ""))
	TT <- rbind(TT, rep(0, states+1))
	diag(TT) <- 0
	...
	nu <- as.list(nu)
	names(nu) <- setdiff(c(t(TT)), c("0"))
	zeta <- as.list(rep(zeta, each=states+1))
	names(zeta) <- setdiff(c(t(TT)), c("0"))
